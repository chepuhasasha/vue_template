...
- При инициализации читает локаль из `localStorage`, иначе использует `DEFAULT_LOCALE` (`en`).
- При смене локали сохраняет значение в `localStorage`.
- Если ключ отсутствует в текущей локали, берёт перевод из `en`, а при отсутствии и там возвращает сам ключ.
