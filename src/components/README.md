...
| `size`     | `'s' \| 'm' \| 'l'`                   | `'m'`        | Размер кнопки (s=20px, m=40px, l=60px).                   |
| `type`     | `'button' \| 'submit' \| 'reset'`     | `'button'`   | Атрибут `type` для элемента `<button>`.                   |
| `rounded`  | `boolean`                             | `false`      | Делает кнопку полностью круглой (pill).                   |
...
- Если передан слот `end`, контент внутри кнопки распределяется по ширине через `space-between`, чтобы правый элемент прижимался к краю.
...
| `size`  | `'s' \| 'm' \| 'l'` | `'m'`        | Размер контейнера (s=20px, m=40px, l=60px) и svg (s=14px, m=18px, l=24px). |
...
| `size`        | `'s' \| 'm' \| 'l'`                                                         | `'m'`        | Размер инпута (высота поля: s=20px, m=40px, l=60px).               |
...
- В вертикальной компоновке (`layout="vertical"`) лейбл отображается в рамке над полем и имеет размер шрифта 10px.
