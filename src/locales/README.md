...
- Источник истины для ключей — `ru.json` (из него формируется тип `TranslationKey`).
- Все ключи должны присутствовать в каждой локали, даже если перевод временно совпадает.
...
- Если ключ отсутствует в текущей локали, используется `DEFAULT_LOCALE`.
