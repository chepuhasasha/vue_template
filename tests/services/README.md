...
| Тест | Описание |
| --- | --- |
| `формирует корректный URL API` | Проверяет сборку URL API из base URL и пути. |
| `разрешает использовать base URL из окружения` | Проверяет использование `VITE_API_BASE_URL` при формировании URL. |
| `добавляет SID в заголовок запроса` | Проверяет подпись запросов SID из хранилища. |
| `не добавляет SID при отключенной подписи` | Проверяет, что запрос отправляется без SID при `includeSessionId=false`. |
| `сохраняет явный заголовок с SID` | Проверяет, что явный заголовок подписи не перезаписывается. |
